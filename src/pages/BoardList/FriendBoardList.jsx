...
const FriendBoardList = () => {
    ...
    const fetchFriendBoardData = async (page) => {
        console.log('fetchBoardData start');
        try {
            const token = getToken(); // 토큰 가져오기
            // const page_number = searchParams.get("page");
            const response = await axios.get(`/api/auth/post/friend?page=${page}&size=8`, {
                headers: {
    ...
    useEffect(() => {
        fetchFriendBoardData(currentPage);
    }, [currentPage])
...
export default FriendBoardList;